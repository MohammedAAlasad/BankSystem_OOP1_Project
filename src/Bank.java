 ...
 public void addSalary(Employee emp) {
    ...
    try (Connection conn = Database.connect();
     ...
     public void addAcc(User user) {
        String sql = "INSERT INTO accounts(user_id, fullname, username, gender, password) VALUES(?, ?, ?, ?, ?)";
...
public void showEmpByAdmin(int emp) {
    ...
    for(Employee e : employees){
            if(empid == e.getID()){
                ...
                System.out.println("salary edit succeffly:");
            }
            else{
                System.out.println("Invalid input. Please enter a correct ID:");
            }
    ...
    // 2. Get balance data
    String balanceSql = "SELECT balance FROM users_Balance WHERE user_id = ?";
